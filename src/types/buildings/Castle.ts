...
export class Castle extends BuildingBase {
  ...
  hire(unit: string): string {
    if (this.availableUnits.includes(unit)) {
      return `Найнято ${unit} в замке (${this.position.x}, ${this.position.y})`;
    }
    return `${unit} недоступен в этом замке`;
  }
  ...
  activate(): void {
    console.log(`Замок (${this.position.x}, ${this.position.y}) активирован. Защита: ${this.defense}`);
  }
