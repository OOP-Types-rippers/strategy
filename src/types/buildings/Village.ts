...
export class Village extends BuildingBase {
  ...
  activate(): void {
    console.log(`Cело (${this.position.x}, ${this.position.y}) активована. Населення: ${this.population}`);
  }
  
  info(): string {
    return `Village Lvl.${this.level} at (${this.position.x}, ${this.position.y}) | Gold: ${this.goldPerTurn}/turn`;
  }
